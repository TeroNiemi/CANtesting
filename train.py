...
model = Sequential()
model.add(Conv1D(32, kernel_size=3, activation='relu', input_shape=(8, 1), padding='same'))  # Padding to maintain input size
model.add(MaxPooling1D(pool_size=2))  # Max pooling layer
model.add(Dropout(0.5))  # Dropout for regularization

model.add(Conv1D(64, kernel_size=3, activation='relu', padding='same'))  # Additional convolutional layer with padding
model.add(MaxPooling1D(pool_size=2))  # Max pooling layer
...
# Train the model
history = model.fit(X_train, y_train_categorical, epochs=400, batch_size=16, validation_split=0.1, callbacks=[early_stopping])
